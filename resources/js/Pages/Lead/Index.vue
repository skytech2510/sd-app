...
const newlead = computed(() => {
    let data = props.leads.data.filter((x) => x.status_id === 9 && x.new_lead === 1);
    return data;   
});
            ...
            <div class="pt-10">
             ...
             <div class="grid sm:grid-cols-12 md:grid-cols-10 md:gap-2 sm:gap-1 md:mt-10 sm:mt-0 mb-5 text-slate-600 ">
                ...
                <div class="flex justify-between w-full">
                    ...
                    <div class="sm:hidden md:block" v-if="$page.props.user.roles[0] === 'admin'">
                        <Button 
                            variant="default" 
                            type="button" 
                    ...
                    <TabPanel>
                            ...
                            <li
                                ...
                                <div class="flex justify-between border-l-4 border-red-600 pl-4">
                                    ...
                                    <div class="flex space-x-1 h-10 z-10">
                                        <Actions :lead="lead" @openProposal="openProposal(lead)"></Actions>
                                    </div>
                                </div>
                                <div class="flex mt-4">
                                ...
                                <div class="justify-between md:flex mt-4">                                
                                    ...
                                    <div class="flex text-slate-400 space-x-2 md:pt-2 sm:pt-1 justify-between">
                                        ...
                                        <span v-if="$page.props.user.roles[0] === 'admin'" class=" pt-0.5 text-xxs cursor-default" title="Usuário Responsável">{{ lead.usuario}}</span>
                        ...
                        <ul class="pt-4">
                            <li 
                                ...
                                <div class="flex mt-2 text-slate-900">
                                    <Badge class="bg-white border border-slate-300 text-slate-600 px-1 py-1 rounded-md sm:text-xxs md:text-xs font-light">{{ lead.funnel_step }}</Badge>
                                    <Badge class="bg-slate-50 text-slate-600 px-1 py-1 rounded-md sm:text-xxs md:text-xs font-light">{{ lead.channel }}</Badge>
                                    <Badge class="bg-slate-50 text-slate-600 px-1 py-1 rounded-md sm:text-xxs md:text-xs font-light">{{ lead.origem }}</Badge>
                                </div>
                                <div class="justify-between md:flex mt-4">   
                                    ...
                                    <div class="flex text-slate-400 space-x-2 md:pt-2 sm:pt-1 justify-between">
                                        ...
                                        <span v-if="$page.props.user.roles[0] === 'admin'" class="pt-0.5 text-slate-400 text-xxs cursor-default">{{ lead.usuario}}</span>
                                    </div>
                                </div>
                            </li>
                        </ul>
                        ...
                        <ul class="pt-2">
                            <li 
                                ...
                                <div class="flex mt-4 ">
                                    <Badge class="bg-white border border-slate-300 text-slate-600 px-1 py-1 rounded-md font-light sm:text-xxs md:text-xs">{{ lead.funnel_step }}</Badge>
                                    <Badge class="bg-slate-50 text-slate-600 px-1 py-1 rounded-md font-light sm:text-xxs md:text-xs">{{ lead.channel }}</Badge>
                                    <Badge class="bg-slate-50 text-slate-600 px-1 py-1 rounded-md font-light sm:text-xxs md:text-xs">{{ lead.origem }}</Badge>
                                </div>
                                <div class="justify-between md:flex mt-4">                                
                                    <div class="flex text-slate-400 justify-between">
                                        <div class="flex">
                                    ...
                                    <div class="flex text-slate-400 space-x-2 md:pt-2 sm:pt-1 justify-between">
                                        <span class="text-slate-400 text-xxs pt-0.5"> Recebido em {{ moment(lead.created_at).format("DD/MM/YYYY HH:mm") }}</span> 
                                        <span v-if="$page.props.user.roles[0] === 'admin'" class="text-xxs pt-0.5">{{ lead.usuario}}</span>
                                    </div>                              
                                </div>
                    ...
                    <TabPanel>
                        <ul class="mt-6">
                            <li 
                                ...
                                :key="lead.id"    
                                class="bg-white px-5 py-5 mb-5 shadow-md">
                                <div :class="`flex justify-between border-l-4 ${lead.status_id === 7 ? 'border-sky-500': lead.status_id === 9 ? 'border-red-600':  lead.status_id === 8 ? 'border-slate-500': lead.status_id === 10 ? 'border-amber-500': ''} pl-4 mb-4`">
                                    <div class="mb-3">
                                        <Link :href="`/lead/${lead.id}/`" class="font-bold text-sky-900">{{ lead.name }}</Link>
                                        <p class="text-xs text-gray-500">{{ lead.product }}</p>
                                    </div>
                                ...
                                <div class="flex mt-4">
                                    <Badge class="bg-white border border-slate-300 text-slate-600 px-1 py-1 rounded-md text-xxs font-light">{{ lead.funnel_step }}</Badge>
                                ...
                                <div class="justify-between md:flex mt-4">                                
                                    <div class="flex text-slate-400 justify-between">
                                        <span class="text-xs" :class="lead.status_id === 7 ? 'text-sky-500 font-semibold mr-2': lead.status_id === 9 ? 'text-red-600 font-semibold mr-2':  lead.status_id === 8 ? 'text-slate-500 font-semibold mr-2': lead.status_id === 10 ? 'text-amber-500 font-semibold mr-2': '' ">{{ lead.status}}</span>
                                        <span v-if="lead.schedule_date && lead.status_id === 7" class="text-xxs pt-0.5"> Retornar em {{ moment(lead.schedule_date).format("DD/MM/YYYY HH:mm") }}</span>
                                        <span v-if="lead.schedule_date === null && lead.status_id === 7" class="text-xxs pt-0.5"> Sem agendamentos</span>
                    ...
                    <TabPanel>
                        <ul class="mt-6">
                            <li 
                                ...
                                <div :class="`flex justify-between border-l-4 ${lead.status_id === 7 ? 'border-sky-500': lead.status_id === 9 ? 'border-red-600':  lead.status_id === 8 ? 'border-slate-500': lead.status_id === 10 ? 'border-amber-500': lead.status_id === 16 ? 'border-green-500': ''} pl-4 mb-4`">
                                    <div class="mb-3">
                                        <Link :href="`/lead/${lead.id}/`" class="font-bold text-sky-900">{{ lead.name }}</Link>
                                        <p class="text-xs text-gray-500">{{ lead.product }}</p>
                                    </div>
                                ...
                                <div class="justify-between md:flex mt-4">                                
                                    <div class="flex text-slate-400 justify-between">
                                        <span class="text-xs" :class="lead.status_id === 7 ? 'text-sky-500 font-semibold mr-2': lead.status_id === 9 ? 'text-red-600 font-semibold mr-2':  lead.status_id === 8 ? 'text-slate-500 font-semibold mr-2': lead.status_id === 10 ? 'text-amber-500 font-semibold mr-2': lead.status_id === 16 ? 'text-green-500 font-semibold mr-2': '' ">{{ lead.status}}</span>
                                        <span v-if="lead.schedule_date && lead.status_id === 7" class="text-xxs pt-0.5"> Retornar em {{ moment(lead.schedule_date).format("DD/MM/YYYY HH:mm") }}</span>
                                        <span v-if="lead.schedule_date === null && lead.status_id === 7" class="text-xxs pt-0.5"> Sem agendamentos</span>
                                        <span v-if="lead.status_id === 8" class="text-xxs pt-0.5"> Última Interação {{ moment(lead.interaction_date).format("DD/MM/YYYY HH:mm") }}</span>
                ...
                <div class="bg-white">
                    <form @submit.prevent="filterLeads">
                        <div class="p-5 py-5 space-y-5">
                            ...
                            <div v-if="$page.props.user.roles[0] === 'admin'"  class="grid grid-cols-3 gap-4">
                                <div class="col-span-3">
                            ...
                            <div v-if="$page.props.user.roles[0] === 'admin'" class="sm:pb-16">
                                <Button 
                                    variant="default" 
                                    type="button" 
