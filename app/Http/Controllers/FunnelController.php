    ...
    public function index(Request $request)
    {
        ...
        foreach ($kabanCards as $step)
        {
            $leads = Lead::join('users', 'leads.user_id', '=', 'users.id')
                        ...
                        ->select('leads.id', 'leads.name', 'leads.celular', 'users.name as usuario', 'products.name as product', 'leads.favorite', 'leads.status_id', 'leads.created_at', 'leads.new_lead', 'status.name as status', 'leads.status_id', 'schedule_date', 'interaction_date','origems.name as origem', 'channels.name as channel', 'funnel_steps.name as funnel_step')
                        ->whereNotIn('leads.status_id', [3])                        
                        ->whereBetween ('leads.created_at', $dates)
                        ->where('leads.funnel_step_id', '=', $step->id);
                        

            $leads->when($request->term, function ($query) use ($request, $findme) {
                if(!strpos($request->term, $findme)){
            ...
            if ($request->has('channel')) {
                $leads->whereIn ('leads.channel_id', $request->channel);
            }
        ...
        if ($request->has('companies')) {
            $users = User::select('id as value', 'name as label')->whereIn('company_id', $request->companies)->whereNotIn('status_id', [3])->orderBy('name');
        }else{
